...
`getUserMedia` оставляет echo-cancellation и AGC. Браузерный noise-suppression включается только для движка **Браузерное**; для worklet-движков он отключён, чтобы не каскадировать два шумодава и не резать речь.
...
| ----------- | ------------------------------------------------------- | --------------------------------------------------- |
| **Браузерное** | встроен в браузер, быстрый fallback без WASM          | качество и поведение зависят от браузера/ОС         |
...
Всё локальное состояние — в `localStorage`: имя, выбранный движок, силы и громкости, mute toggles, hotkey. Никаких аккаунтов на бэкенде.
